...
from melange.drivers.kombu_driver import KombuDriver


@pytest.fixture
def kombu_driver():
    return KombuDriver(
        "redis://localhost:6379", wait_time_seconds=1, visibility_timeout=10
    )


@pytest.fixture
def topic(kombu_driver):
    topic = kombu_driver.declare_topic("dev-topic-{}".format(uuid.uuid4()))
    yield topic
    kombu_driver.delete_topic(topic)


def test_create_queue(kombu_driver, request):
    def delete_queue():
        kombu_driver.delete_queue(queue)

    request.addfinalizer(delete_queue)

    queue_name = "test-queue-{}".format(uuid.uuid4())
    queue, _ = kombu_driver.declare_queue(queue_name)
    queue = kombu_driver.get_queue(queue_name)
...
def test_create_fifo_queue_with_dlq(kombu_driver, request):
    def delete_queue():
        kombu_driver.delete_queue(queue)
        kombu_driver.delete_queue(dlq)

    request.addfinalizer(delete_queue)

    queue_name = "test-queue-{}.fifo".format(uuid.uuid4())
    dlq_queue_name = "test-queue-{}-dlq.fifo".format(uuid.uuid4())
    queue, dlq = kombu_driver.declare_queue(
    ...
    assert_that(
        dlq.attributes, has_entries(FifoQueue="true", ContentBasedDeduplication="false")
    )


def test_create_a_non_fifo_queue(kombu_driver, request):
    def delete_queue():
        kombu_driver.delete_queue(queue)
    ...
    queue_name = "test-queue-{}".format(uuid.uuid4())
    kombu_driver.declare_queue(queue_name)
    queue = kombu_driver.get_queue(queue_name)
...
def test_create_normal_queue_with_dlq(kombu_driver, request):
    def delete_queue():
        kombu_driver.delete_queue(queue)
        kombu_driver.delete_queue(dlq)
    ...
    assert_that(
        dlq.attributes,
        not_(has_entries(FifoQueue="true", ContentBasedDeduplication="false")),
    )


def test_send_messages_through_a_fifo_queue_and_make_sure_they_are_always_ordered(
    kombu_driver, request
):
    def delete_queue():
        kombu_driver.delete_queue(queue)
...
def test_send_messages_through_a_non_fifo_queue_does_not_guarantee_order(
    ...
    for i in range(100):
        kombu_driver.queue_publish(f"my-message-{i}", queue, "my-event-type")
...
def test_kombu_driver_declare_a_queue_for_a_topic_filtering_the_events_it_sends_to_certain_queues(
    kombu_driver, topic
):
    try:
        queue_1, _ = kombu_driver.declare_queue(
            "dev-queue-{}".format(uuid.uuid4()), topic, filter_events=["MyBananaEvent"]
        )
        queue_2, _ = kombu_driver.declare_queue(
            "dev-queue-{}".format(uuid.uuid4()),
            topic,
            filter_events=["MyNonBananaEvent"],
        )
        queue_3, _ = kombu_driver.declare_queue(
            "dev-queue-{}".format(uuid.uuid4()), topic
        )

        kombu_driver.publish(
        ...
        _assert_messages(
            kombu_driver, queue_3, ["MyNonBananaEvent", "MyBananaEvent"], 2
        )
        _assert_messages(kombu_driver, queue_1, ["MyBananaEvent"], 1)
        _assert_messages(kombu_driver, queue_2, ["MyNonBananaEvent"], 1)

    finally:
        kombu_driver.delete_queue(queue_1)
        kombu_driver.delete_queue(queue_2)
        kombu_driver.delete_queue(queue_3)


def _assert_messages(
    kombu_driver, queue, expected_types_contained, expected_num_messages
):
    messages = []
    while True:
        retrieved_messages = kombu_driver.retrieve_messages(queue)
...
class TestAWSDriver:
    ...
    def test_create_queue_and_bind_to_topic(self):
        ...
        policy_topic_arn = policy["Statement"][0]["Condition"]["ArnEquals"][
            "aws:SourceArn"
        ]
    ...
    def test_create_queue_and_bind_several_topics(self):
        ...
        self.queue, _ = self.driver.declare_queue(
            self._get_queue_name(), self.topic_1, self.topic_2
        )
        ...
        policy_topic_arn = policy["Statement"][1]["Condition"]["ArnEquals"][
            "aws:SourceArn"
        ]
