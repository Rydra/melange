...
def test_create_fifo_queue_with_dlq(aws_driver, request):
    ...
    queue, dlq = aws_driver.declare_queue(
...
def test_send_messages_through_a_non_fifo_queue_does_not_guarantee_order(
    ...
    for i in range(100):
        aws_driver.queue_publish(f"my-message-{i}", queue, "my-event-type")
...
def test_aws_driver_declare_a_queue_for_a_topic_filtering_the_events_it_sends_to_certain_queues(
    aws_driver, topic
):
    try:
        queue_1, _ = aws_driver.declare_queue(
            "dev-queue-{}".format(uuid.uuid4()), topic, filter_events=["MyBananaEvent"]
        )
        queue_2, _ = aws_driver.declare_queue(
            "dev-queue-{}".format(uuid.uuid4()),
            topic,
            filter_events=["MyNonBananaEvent"],
        )
        queue_3, _ = aws_driver.declare_queue(
...
def _assert_messages(
    aws_driver, queue, expected_types_contained, expected_num_messages
):
    messages = []
    while True:
        retrieved_messages = aws_driver.retrieve_messages(queue)
