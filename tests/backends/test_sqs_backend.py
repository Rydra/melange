...
@pytest.fixture
def topic(aws_backend):
    topic = aws_backend.declare_topic("dev-topic-{}".format(uuid.uuid4()))
    yield topic
    aws_backend.delete_topic(topic)


def test_create_fifo_queue(aws_backend, request):
    def delete_queue():
        aws_backend.delete_queue(queue)

    request.addfinalizer(delete_queue)

    queue_name = "test-queue-{}.fifo".format(uuid.uuid4())
    aws_backend.declare_queue(queue_name)
    queue = aws_backend.get_queue(queue_name)
...
def test_create_fifo_queue_with_dlq(aws_backend, request):
    def delete_queue():
        aws_backend.delete_queue(queue)
        aws_backend.delete_queue(dlq)

    request.addfinalizer(delete_queue)

    queue_name = "test-queue-{}.fifo".format(uuid.uuid4())
    dlq_queue_name = "test-queue-{}-dlq.fifo".format(uuid.uuid4())
    queue, dlq = aws_backend.declare_queue(
...
def test_create_a_non_fifo_queue(aws_backend, request):
    def delete_queue():
        aws_backend.delete_queue(queue)
...
def test_create_normal_queue_with_dlq(aws_backend, request):
    def delete_queue():
        aws_backend.delete_queue(queue)
        aws_backend.delete_queue(dlq)
...
def test_aws_backend_declare_a_queue_for_a_topic_filtering_the_events_it_sends_to_certain_queues(
    aws_backend, topic
):
    try:
        queue_1, _ = aws_backend.declare_queue(
            "dev-queue-{}".format(uuid.uuid4()), topic, filter_events=["MyBananaEvent"]
        )
        queue_2, _ = aws_backend.declare_queue(
            "dev-queue-{}".format(uuid.uuid4()),
            topic,
            filter_events=["MyNonBananaEvent"],
        )
        queue_3, _ = aws_backend.declare_queue(
        ...
        _assert_messages(aws_backend, queue_3, ["MyNonBananaEvent", "MyBananaEvent"], 2)
        _assert_messages(aws_backend, queue_1, ["MyBananaEvent"], 1)
        _assert_messages(aws_backend, queue_2, ["MyNonBananaEvent"], 1)

    finally:
        aws_backend.delete_queue(queue_1)
        aws_backend.delete_queue(queue_2)
        aws_backend.delete_queue(queue_3)


def _assert_messages(
    aws_backend, queue, expected_types_contained, expected_num_messages
):
    messages = []
    while True:
        retrieved_messages = aws_backend.retrieve_messages(queue)
...
class TestAWSBackend:
    ...
    def test_create_normal_queue(self):
        self.queue, _ = self.backend.declare_queue(self._get_queue_name())
        assert self.queue.url
        assert "Policy" not in self.queue.attributes

    def test_create_queue_and_bind_to_topic(self):
        self.topic = self.backend.declare_topic(self._get_topic_name())
        self.queue, _ = self.backend.declare_queue(self._get_queue_name(), self.topic)
    ...
    def test_create_queue_and_bind_several_topics(self):
        self.topic_1 = self.backend.declare_topic(self._get_topic_name())
        self.topic_2 = self.backend.declare_topic(self._get_topic_name())
        self.queue, _ = self.backend.declare_queue(
    ...
    def test_create_queue_with_dead_letter_queue(self):
        self.topic = self.backend.declare_topic(self._get_topic_name())
        self.queue, self.dead_letter_queue = self.backend.declare_queue(
